...
#           print(fitData[i])
        ...
        elif self.methodIndex==2: #diffusion fit
            self.updateprogress.emit('Starting Relaxation Fit - diffusion, mean square gradients')     
                       
            def DF0(Tec,R0,D,G0):
                gamma=2.675e8
                D=2e3
                #D=2 um/ms, fixed
                return R0 + (Tec**3 * D * gamma**2 * G0**2)/12.0
...
#            print(fitPar)
            ...
            if numFixed==0:
                popt,pcov=opt.curve_fit(DF0, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=guesses)
                fitPar=popt
                fitParpm[0]=np.abs(pcov[0,0]**0.5)
                fitParpm[1]=np.abs(pcov[1,1]**0.5)
                fitParpm[2]=np.abs(pcov[2,2]**0.5)
                    ...
                    if self.fixedPar[0]:
                        DF1 = lambda x,a,b: DF0(x, self.fixedParVal[0], a, b)
                        popt,pcov= opt.curve_fit(DF1, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=[guesses[1],guesses[2]])
                        fitPar[1]=popt[0]
                        fitPar[2]=popt[1]
                        fitParpm[1]=np.abs(pcov[0,0]**0.5)
                        fitParpm[2]=np.abs(pcov[1,1]**0.5)
                        ...
                        DF1 = lambda x,a,b: DF0(x, a, self.fixedParVal[1], b)
                        popt,pcov= opt.curve_fit(DF1, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=[guesses[0],guesses[2]])
                        fitPar[0]=popt[0]
                        fitPar[2]=popt[1]
                        fitParpm[0]=np.abs(pcov[0,0]**0.5)
                        fitParpm[2]=np.abs(pcov[1,1]**0.5)
                        ...
                        DF1 = lambda x,a,b: DF0(x, a, b, self.fixedParVal[2])
                        popt,pcov= opt.curve_fit(DF1, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=[guesses[0],guesses[1]])
                        fitPar[0]=popt[0]
                        fitPar[1]=popt[1]
                        fitParpm[0]=np.abs(pcov[0,0]**0.5)
                        fitParpm[1]=np.abs(pcov[1,1]**0.5)
                ...
                if self.fixedPar[0]:
                    if self.fixedPar[1]:
                        DF2 = lambda x,a: DF0(x, self.fixedParVal[0], self.fixedParVal[1], a)
                        popt,pcov= opt.curve_fit(DF2, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=guesses[2])
                        fitPar[2]=popt
                        fitParpm[2]=np.abs(pcov**0.5)
                    
                    else:
                        DF2 = lambda x,a: DF0(x, self.fixedParVal[0], a, self.fixedParVal[2])
                        popt,pcov= opt.curve_fit(DF2, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=guesses[1])
                        fitPar[1]=popt
                        fitParpm[1]=np.abs(pcov**0.5)
                else:
                    DF2 = lambda x,a: DF0(x, a, self.fixedParVal[1], self.fixedParVal[2])
                    popt,pcov= opt.curve_fit(DF2, self.echoTimes, self.R2, sigma=self.R2pm, absolute_sigma=absSig, p0=guesses[0])
                    fitPar[0]=popt
                    fitParpm[0]=np.abs(pcov**0.5)
            ...
            self.bgThreadTextOut.emit('Fit R2 values to R2 = R0 + (Tec**3 * D * gamma**2 * G**2)/12' )
            self.bgThreadTextOut.emit('%d/3 Fixed parameters' % numFixed)
            if numFixed>0:
                self.bgThreadTextOut.emit('** = Fixed value')
            self.bgThreadTextOut.emit('|{0:^10.10s}|{1:^10.10s}|{2:^10.10s}|{3:^10.10s}|{4:^10.10s}|{5:^10.10s}|'.format('R0' + printTrue(self.fixedPar[0]),'+/-','D' + printTrue(self.fixedPar[1]),'+/-','G' + printTrue(self.fixedPar[2]), '+/-'))
            fitString='|{0:^10.4f}|{1:^10.4f}|{2:^10.4f}|{3:^10.4f}|{4:^10.4e}|{5:^10.4e}|'.format(fitPar[0], fitParpm[0], fitPar[1], fitParpm[1], fitPar[2], fitParpm[2])
